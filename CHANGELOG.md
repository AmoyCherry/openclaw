...
- Gateway/Control UI: fix trusted-proxy auth mode being bypassed for Control UI WebSocket connections — device pairing is now skipped when the gateway authenticates the request via a trusted proxy, matching the behavior for CLI connections.
