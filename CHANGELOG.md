...
- Gateway/Control UI: when `gateway.auth.mode` is `trusted-proxy`, Control UI connections now honor the proxy-vouched user identity and skip device pairing instead of failing with "pairing required", matching CLI behavior behind a reverse proxy.
